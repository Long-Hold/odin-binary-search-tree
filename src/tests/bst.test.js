...
describe('class Tree', () => {
  ...
  describe('Tree.insert()', () => {
    test('inserts a value into the tree on the left and right of a node', () => {
      ...
      for (let i = 0; i < arr.length; ++i)
        expect(newTree.includes(arr[i])).toBe(true);
      newTree.insert(15);
      newTree.insert(17);
  ...
  describe('Tree.deleteItem()', () => {
    test('deletes node with no children', () => {
      ...
      for (let i = 0; i < arr.length; ++i)
        expect(noChildTree.includes(arr[i])).toBe(true);
      ...
      for (let i = 0; i < newArr.length; ++i)
        expect(noChildTree.includes(newArr[i])).toBe(true);
    });
    test('deleted node has 1 child', () => {
      const arr = [50, 30, 70, 20, 40, 60];
      const oneChildTree = new Tree(arr);
      for (let i = 0; i < arr.length; ++i)
        expect(oneChildTree.includes(arr[i])).toBe(true);
      ...
      const newArr = arr.filter((item) => item !== valueToRemove);
      for (let i = 0; i < newArr.length; ++i)
        expect(oneChildTree.includes(newArr[i])).toBe(true);
    });
    test('deleted node has 2 children, or is the root node', () => {
      const arr = [50, 30, 70, 20, 40, 60, 80];
      const twoChildTree = new Tree(arr);
      for (let i = 0; i < arr.length; ++i)
        expect(twoChildTree.includes(arr[i])).toBe(true);
      ...
      const newArr = arr.filter((item) => item !== valueToRemove);
      for (let i = 0; i < newArr.length; ++i)
        expect(twoChildTree.includes(newArr[i])).toBe(true);
    });
  ...
  describe('Tree.levelOrderForEach()', () => {
    ...
    test.each([1, [], 'name', {}])(
      'throws TypeError when passed "%s" to callBack parameter',
      (input) => {
